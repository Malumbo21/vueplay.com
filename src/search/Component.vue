...
<script>
	...
	export default {
		...
		methods: {
			async getCategories() {
				const categories = await this.io.service("types/categories")
					.find();
				this.categories = categories?.data || [];
			},
			async postComment() {
				if (await this.login()) {
					await this.io.service("types/comments")
						.create({
			...
			async savePost() {
				await this.io.service("types/applications")
					.patch(this.post._id, {
			...
			async refresh() {
				this.post = await this.io.service("types/applications")
					.get(this.id);
				this.getCategories();
				this.$emit("post", this.post);
			},
			async myVote(post) {
				const votes = await this.io.service("types/votes")
					.find({
						query: {
			...
			async vote(post) {
				if (await this.login()) {
					...
					if (myVote) {
						await this.io.service("types/votes")
							.remove(myVote._id);
					} else {
						await this.io.service("types/votes")
							.create({
			...
			async refreshVotes(post) {
				post.votes = await this.io.service("types/votes")
					.find({
						query: {
							application_id: post._id
						}
			...
			async myLike(comment) {
				const likes = await this.io.service("types/likes")
					.find({
						query: {
			...
			async like(comment) {
				if (await this.login()) {
					...
					if (myLike) {
						await this.io.service("types/likes")
							.remove(myLike._id);
					} else {
						await this.io.service("types/likes")
							.create({
			...
			async refreshLikes(comment) {
				comment.likes = await this.io.service("types/likes")
					.find({
						query: {
							comment_id: comment._id
						}
