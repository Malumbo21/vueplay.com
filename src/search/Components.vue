<template>
    <section class="p-6 md:pt-6 pb-24">
            ...
            <div class="lg:grow">
                <div class="lg:float-right mt-4 lg:mt-0 lg:h-full">
                    <div class="relative lg:h-full">
                        ...
                        </div> <input type="search" id="default-search" class="w-full h-10 lg:w-52 lg:h-full max-w-full pl-10 pr-2 text-sm text-gray-900 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search..." required="" v-model="term" @change="refresh()" />
                    </div>
                </div>
            </div>
        </div>
        <article v-for="post in posts.data" class="max-w-2xl mx-auto mb-8 mt-4 flex cursor-pointer">
            ...
            <div class="hover:text-emerald-600 overflow-hidden text-ellipsis inline-flex flex-col grow pr-4 cursor-pointer" @click="$router.push('/search/' + post._id)" post.over="" style="max-height:75px;height:1806.03px">
            ...
            </div><img class="rounded w-16 h-16" :src="post.icon || background" />
            <div class="pl-4 inline-flex cursor-pointer" @click="$router.push('/search/' + post._id)" @mouseenter="post.over = true" @mouseleave="post.over = false">
