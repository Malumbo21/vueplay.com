                ...
                <div class="lg:float-right mt-4 lg:mt-0 lg:h-full">
                    <div class="relative lg:h-full">
                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                            ...
                            class="w-full h-10 lg:w-52 lg:h-full max-w-full pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        <article v-for="post in posts" class="max-w-2xl mx-auto mb-8 mt-4 flex">
            <div class="w-16 inline-flex">
                <div class="cursor-pointer border w-11 h-12 rounded-lg" @click="vote(post)"
                    :class="{ 'bg-emerald-50': post.votes.find(v => v.user_id === user.value._id), 'border-emerald-400': post.votes.find(v => v.user_id === user.value._id), 'hover:bg-emerald-100': post.votes.find(v => v.user_id === user.value._id), 'hover:bg-slate-100': !post.votes.find(v => v.user_id === user.value._id), 'border-emerald-400': post.votes.find(v => v.user_id === user.value._id) }">
                    <div class="h-1/2 w-full">
                        <svg viewBox="0 0 20 10" class="pt-2 w-5 h-5 mx-auto"
                            :class="{ 'fill-emerald-400': post.votes.find(v => v.user_id === user.value._id), 'fill-gray-400': !post.votes.find(v => v.user_id === user.value._id) }">
            ...
            <div class="overflow-hidden text-ellipsis max-h-20 flex-col inline-flex grow mr-4 cursor-pointer"
                @click="$router.push('/feedback/' + post._id)">
            ...
            <div class="w-12 inline-flex">
                <div class="w-1/2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor" class="stroke-gray-400 w-5 h-5 mt-1">
