<template>
	<div class="bg-slate-50 rounded" style="max-width:100%;max-height:100%" :class="fullWindow ? 'full-window' : ''">
		<div class="grid-cols-2 grid bg-slate-100 pt-1 px-1" style="width:100%;height:40px">
			<div class="h-full">
				<button style="width:31px;height:31px"
					...
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="mx-auto"
				...
				<button style="width:31px;height:31px"
					class="bg-slate-200 m-0.5 mr-0 rounded text-center inline-block disabled:opacity-40 hover:bg-slate-300"
					:disabled="user?._id !== application?.user_id || saving" @click="save()">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="mx-auto"
						style="width:80%;height:80%;padding:10%">
						<path fill-rule="evenodd"
			...
			<div class="text-right h-full">
				<button v-if="application?.tailwind" style="width:31px;height:31px"
					class="bg-slate-200 m-0.5 mr-0 rounded text-center inline-block" title="Component depends on Tailwind">
				...
				<button style="width:31px;height:31px"
					class="bg-slate-200 m-0.5 mr-0 rounded text-center inline-block hover:bg-slate-300" title="Open preview"
				...
				<button v-if="!fullWindow" style="width:31px;height:31px"
					class="bg-slate-200 m-0.5 mr-0 rounded text-center inline-block hover:bg-slate-300" title="Full window"
				...
				<button v-if="fullWindow" style="width:31px;height:31px"
					class="bg-slate-200 m-0.5 rounded text-center inline-block hover:bg-slate-300" title="Close full window"
		...
		<div class="flex-row flex bg-slate-50" style="width:100%;height:calc(100% - 40px)">
			<div class="grid-cols-2 grid grow">
				<div v-show="show === 'split' || show === 'code'" class="p-4 overflow-auto h-full"
					:class="show !== 'split' ? 'col-span-2' : ''">
					<div class="editor" ref="editor" />
				</div>
				<div v-show="show === 'split' || show === 'preview'" class="p-1 overflow-auto h-full"
					:class="show !== 'split' ? 'col-span-2' : ''">
					<iframe class="bg-white h-full w-full" ref="preview" :uid="id"
						:src="'https://studio.vueplay.com/preview#' + id" />
				</div>
			</div>
			<div v-if="showHistory" class="relative px-4 pb-4 box-border overflow-auto h-full"
				...
				<div v-for="commit in history" class="p-2 rounded mb-2 bg-slate-100">
...
import *
	as onigasm from "https://cdn.jsdelivr.net/npm/onigasm@2.2.5/+esm"
import *
	as monaco from "https://cdn.jsdelivr.net/npm/monaco-editor-core-colorpicker@0.38.0/+esm"
...
function loadOnigasm() {
	...
	return onigasm.loadWASM(
		"https://cdn.jsdelivr.net/npm/onigasm@2.2.5/lib/onigasm.wasm"
	)
}
let vueWorker = async () => {
	let url =
		"https://cdn.jsdelivr.net/npm/monaco-volar@0.4.0/dist/worker/vue.worker.js"
	...
	*/
	...
	async function setup() {
		...
		window.MonacoEnvironment.getWorker =
			async () => editorWorker()
		const getWorker = window.MonacoEnvironment
			.getWorker
		window.MonacoEnvironment.getWorker =
			async (_, label) => {
				if (label === "vue")
					return await vueWorker()
				return getWorker()
			}
		const worker = monaco.editor
			.createWebWorker({
		...
		const getSyncUris = () => monaco.editor
			.getModels()
			.map(model => model.uri)
		volar.editor.activateMarkers(worker,
			languageId, "vue", getSyncUris,
			monaco.editor)
		volar.editor.activateAutoInsertion(worker,
			languageId, getSyncUris, monaco
			.editor)
		await volar.languages.registerProvides(
			worker, languageId, getSyncUris,
			monaco.languages)
	}
	await setup()
}
export default {
	...
	async created() {
		...
		await this.init()
		if (!this.io) return
		const apps = this.io.service(
			"types/applications")
		apps.on("updated", data => {
			if (String(data._id) === String(
	...
	watch: {
		raw(raw) {
			this.$refs.preview.contentWindow
				.postMessage({ raw }, '*')
		}
	},
	methods: {
		...
		async refresh() {
			this.application = await this
				.getApplication()
			this.history = await this.getHistory()
			this.raw = await this.getRaw()
			this.preview = await this.getPreview(this
		...
		async loadEditor() {
			const afterReady = async theme => {
				...
				const t = editorInstance
					._themeService._theme
			...
			Promise.all([setupMonacoEnv(),
			loadOnigasm(), loadTheme(monaco
				.editor)])
				.then(([, , theme]) => {
		...
		async save() {
			this.saving = true
			try {
				let results = await this.io.service(
					"types/sfc")
					.patch(this.id, {
						content: this.raw,
						token: await this.io
				...
				alert(
					"You do not have permission to make patches")
			}
			this.saving = false
		}
	}
