<template>
	<div class="bg-slate-50 rounded"
		...
		<div class="grid-cols-2 grid bg-slate-100"
			style="width:100%;height:36px">
			<div class="h-full">
				<button
					style="height:31px"
					class="px-1 text-sm disabled:opacity-40 bg-slate-200 m-0.5 rounded text-center"
					@click="show = 'split'"
					:disabled="show === 'split'"
					v-show="fullWindow">
					Split
			...
			<div
				...
				<button
					...
					class="bg-slate-200 m-0.5 rounded text-center inline-block"
				...
				<button
					...
					@click="openPreview(application?.tailwind)"><svg
		...
		<div class="flex-row flex bg-slate-50"
			style="width:100%;height:calc(100% - 140px)">
			<div
				class="grid-cols-2 grid grow">
				...
				<div class="p-1 overflow-auto h-full"
					:class="show !== 'split' ? 'col-span-2' : ''"
			...
			<div class="relative px-4 pb-4 box-border overflow-auto h-full"
				style="max-width:280px;min-width:280px"
				v-if="showHistory">
				<label for=""
					class="top-0 pt-2 pb-1 sticky w-full bg-slate-50 font-semibold inline-block mb-2">
					Git History
				</label>
				<div class="p-2 rounded mb-2 bg-slate-100"
		...
		<div class="overflow-auto bg-slate-100 p-4"
			...
			<div class="bg-violet-200 mr-1 mb-1 py-0.5 px-3 rounded-xl inline-block"
				v-for="method in application?.analyze?.methods"
				title="Method">
				{{ method }}()
			</div>
			<div class="bg-emerald-200 mr-1 mb-1 py-0.5 px-3 rounded-xl inline-block"
				v-for="injection in application?.analyze?.inject"
				title="Injection">
				{{ injection }}
			</div>
