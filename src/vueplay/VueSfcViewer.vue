<template>
    <div class="bg-slate-50 rounded" style="max-width:100%;max-height:100%" :class="fullWindowClass">
        <div class="grid-cols-2 grid bg-slate-100" style="width:100%;height:40px">
            <div class="h-full">
                <button style="height:31px" class="disabled:opacity-40 px-2 bg-slate-200 m-1 rounded text-center" @click="show = 'split'" :disabled="show === 'split'">
                    Split
                </button><button style="height:31px" class="disabled:opacity-40 px-2 bg-slate-200 m-1 rounded text-center hover:bg-slate-300" @click="show = 'preview'" :disabled="show === 'preview'">
                    Preview
                </button><button style="height:31px" class="disabled:opacity-40 px-2 bg-slate-200 m-1 rounded text-center hover:bg-slate-300" @click="show = 'code'" :disabled="show === 'code'">
                    Code
                </button>
            </div>
            <div class="text-right h-full"><button style="width:31px;height:31px" class="bg-slate-200 m-1 rounded text-center inline-block" v-if="application?.tailwind" title="Component depends on Tailwind">
                    ...
                    </svg> </button><button style="width:31px;height:31px" class="bg-slate-200 m-1 rounded text-center hover:bg-slate-300 inline-block" title="History" @click="showHistory = !showHistory" :class="showHistory ? 'opacity-50' : ''">
                ...
                <button style="width:31px;height:31px" class="bg-slate-200 m-1 rounded text-center inline-block hover:bg-slate-300" v-if="!fullWindow" title="Full window" @click="fullWindow = true"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="mx-auto" style="width:80%;height:80%;padding:10%">
                        <path fill-rule="evenodd" d="M15 3.75a.75.75 0 01.75-.75h4.5a.75.75 0 01.75.75v4.5a.75.75 0 01-1.5 0V5.56l-3.97 3.97a.75.75 0 11-1.06-1.06l3.97-3.97h-2.69a.75.75 0 01-.75-.75zm-12 0A.75.75 0 013.75 3h4.5a.75.75 0 010 1.5H5.56l3.97 3.97a.75.75 0 01-1.06 1.06L4.5 5.56v2.69a.75.75 0 01-1.5 0v-4.5zm11.47 11.78a.75.75 0 111.06-1.06l3.97 3.97v-2.69a.75.75 0 011.5 0v4.5a.75.75 0 01-.75.75h-4.5a.75.75 0 010-1.5h2.69l-3.97-3.97zm-4.94-1.06a.75.75 0 010 1.06L5.56 19.5h2.69a.75.75 0 010 1.5h-4.5a.75.75 0 01-.75-.75v-4.5a.75.75 0 011.5 0v2.69l3.97-3.97a.75.75 0 011.06 0z" clip-rule="evenodd" />
                    </svg>
                </button><button style="width:31px;height:31px" class="bg-slate-200 m-1 rounded text-center inline-block hover:bg-slate-300" v-if="fullWindow" title="Close full window" @click="fullWindow = false"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="mx-auto" style="width:80%;height:80%;padding:10%">
        ...
        <div class="flex-row flex bg-slate-50" style="width:100%;height:calc(100% - 140px)">
            <div class="grid-cols-2 grid grow">
                <div class="p-4 overflow-auto h-full" v-if="show === 'split' || show === 'code'" :class="show !== 'split' ? 'col-span-2' : ''">
                    <pre>
    <code>
    {{ raw }}
    </code> </pre>
                </div>
                <div class="p-4 overflow-auto h-full" v-if="show === 'split' || show === 'preview'" :class="show !== 'split' ? 'col-span-2' : ''"><iframe title="" class="bg-white h-full w-full" :src="iframeSource">
                    </iframe> </div>
            </div>
    ...
    export default {
        ...
        computed: {
            ...
            fullWindowClass() {
                ...
                return "fixed top-0 left-0 w-full h-full"
            }
        },
        async created() {
            await this.init()
