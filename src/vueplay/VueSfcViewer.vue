<template>
    <div class="bg-slate-50 rounded" style="max-width:100%;max-height:100%" :class="fullWindow ? 'full-window' : ''">
        <div class="grid-cols-2 grid bg-slate-100" style="width:100%;height:36px">
            <div class="h-full">
                <button style="height:31px" class="px-1 text-sm disabled:opacity-40 bg-slate-200 m-0.5 rounded text-center" @click="show = 'split'" :disabled="show === 'split'">
                    Split
            ...
            <div class="text-right h-full"><button style="width:31px;height:31px" class="bg-slate-200 m-0.5 rounded text-center inline-block" v-if="application?.tailwind" title="Component depends on Tailwind">
        ...
        <div class="flex-row flex bg-slate-50" style="width:100%;height:calc(100% - 140px)">
            <div class="grid-cols-2 grid grow">
                ...
                <div class="p-4 overflow-auto h-full" :class="show !== 'split' ? 'col-span-2' : ''" v-show="show === 'split' || show === 'preview'"><iframe title="" class="bg-white h-full w-full" :src="iframeSource">
                    </iframe> </div>
            </div>
        ...
        <div class="overflow-auto bg-slate-100 p-4" style="width:100%;height:100px">
            ...
            <div class="bg-violet-200 py-0.5 px-3 rounded-xl inline-block" v-for="prop in application?.analyze?.methods" title="Method">
                {{ method }}()
            </div>
            <div class="bg-emerald-200 py-0.5 px-3 rounded-xl inline-block" v-for="injection in application?.analyze?. inject" title="Injection">
                {{ injection }}
            </div>
...
<script>
    export default {
        ...
        async created() {
            ...
            apps.on("updated", (data, context) => console.log("updated", data, context));
            apps.on("patched", (data, context) => console.log("patched", data, context))
        },
        methods: {
            async init() {
